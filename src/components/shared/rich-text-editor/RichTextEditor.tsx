...
const RichTextEditor = (props: {
  ...
  const getAnimationClasses = (attrs: any) => {
    ...
    else if (animationDuration === "1000ms")
      classes.push("animate-duration-1000");
    else if (animationDuration === "2000ms")
  ...
  const addImage = useCallback(
    (url: string, mode: "inline-block" | "block") => {
  ...
  const addTextarea = useCallback(
    ...
    (textareaConfig: any) => {
      if (editor) {
        editor.commands.insertTextarea(textareaConfig);
    ...
    <div className="max-w-6xl mx-auto p-4">
      <div className="border rounded-lg shadow-lg bg-white">
        ...
        html={value}
          ...
          setShowTextareaDialog={setShowTextareaDialog}
        />
      ...
      {showTextareaDialog && (
        <TextareaDialog
          onConfirm={addTextarea}
          onCancel={() => setShowTextareaDialog(false)}
        />
      )}
    </div>
  );
